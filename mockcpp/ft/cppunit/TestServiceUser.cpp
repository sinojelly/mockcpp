...
class TestService : public CPPUNIT_NS::TestFixture
{
	...
	CPPUNIT_TEST( test_func6_should_invoke_service_f7_with_invoke);
...
public:
   ...
   void test_func6_should_invoke_service_f7_as_simple_char()
   {
     ...
     MOCKER(service_f7)
       .expects(once())
       .with(outBoundP( &c0
                      , eq(&c1))) // has to specify type char*
       .will(returnValue(1));
   ...
   void test_func6_should_invoke_service_f7_with_invoke()
   {
      char* s0 = (char*)"abcdefg";
      char  s1[20];
      ...
      MOCKER(service_f7)
        .expects(once())
        .with(smirror(s1))
        .will(invoke(__service_f7_stub));

      CPPUNIT_ASSERT_EQUAL(101, func6(s0));
   }
