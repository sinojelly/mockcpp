...
$INSTALL_DIR="D:\Develop\work\test-tools\mockcpp"
...
function buildMinGW($build_dir, $src_dir) { 
	if (!(test-path $build_dir)) { mkdir $build_dir }
	cd $build_dir
	Invoke-Expression "cmake -DCMAKE_INSTALL_PREFIX=$INSTALL_DIR $src_dir $MINGW_VER"
	make install
}

#build ..\build_mockcpp_to_install ..\mockcpp
buildMinGW ..\build_mockcpp_to_install ..\mockcpp
